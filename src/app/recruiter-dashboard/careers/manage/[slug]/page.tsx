...
import JobDescriptionV2 from "@/lib/components/CareerComponents/JobDescriptionV2";
...
export default function ManageCareerPage() {
  ...
  const [formData, setFormData] = useState<any>({
    ...
    preScreeningQuestions: undefined,
    aiScreeningSetting: "",
    ...
    const fetchCareer = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          // CV Review fields
          cvScreeningSetting:
            response.data?.cvScreeningSetting ||
            response.data?.screeningSetting ||
            "",
          cvSecretPrompt: response.data?.cvSecretPrompt || undefined,
          preScreeningQuestions:
            response.data?.preScreeningQuestions || undefined,
          // AI Interview fields
          aiScreeningSetting:
            response.data?.aiScreeningSetting ||
            response.data?.screeningSetting ||
            "",
          aiSecretPrompt: response.data?.aiSecretPrompt || undefined,
          // Pipeline stages
  ...
  const handleCancelEdit = () => {
    const deepCopy = JSON.parse(JSON.stringify(career?.questions ?? []));
    setFormData({
      _id: career?._id || "",
      ...
      minimumSalary: career?.minimumSalary || 0,
      maximumSalary: career?.maximumSalary || 0,
      ...
      cvScreeningSetting:
        career?.cvScreeningSetting || career?.screeningSetting || "",
      cvSecretPrompt: career?.cvSecretPrompt || undefined,
      preScreeningQuestions: career?.preScreeningQuestions || undefined,
      aiScreeningSetting:
        career?.aiScreeningSetting || career?.screeningSetting || "",
      aiSecretPrompt: career?.aiSecretPrompt || undefined,
